...
def train_step(data_set, model, optimizer, criterion, epoch):
    losses = []
    progress_bar = tqdm(enumerate(data_set), desc=f'Training epoch {epoch}', total=len(data_set))
...
def test_step(data_set, model, criterion, epoch):
    losses = []
    ...
    progress_bar = tqdm(enumerate(data_set), desc=f'Testing epoch {epoch}', total=len(data_set))
    for i, (x, y) in progress_bar:
        f_x = model(x)
        loss = criterion(f_x, y)
        losses.append(loss.item())
...
def write_log_dir(logs_dir, cluster_name, config):
    model_log_dir = f'{cluster_name}_lr={str(config["lr"])}_' \
...
def train_nn_model(train_dataset, test_dataset, cluster_name, save_paths, config, num_epochs=3,
                   early_stopping_threshold=10, load_checkpoint=False, remove_log_dir=False):
    lr = config['lr']
    l1 = config['l1']
    ...
    saved_models_dir = save_paths['model']
    ...
    log_dir = write_log_dir(save_paths['log'], cluster_name, config)
    ...
    if load_checkpoint:
        min_test_loss, _ = test_step(test_dataset, model, criterion, 0)
    else:
    ...
    for e in range(num_epochs):
        ...
        model.train()
        train_loss = train_step(train_dataset, model, optimizer, criterion, e)

        scheduler.step(train_loss)

        if len(test_dataset) > 0:
            model.eval()
            test_loss, predictions = test_step(test_dataset, model, criterion, e)
        ...
        if train_loss < min_test_loss:
            torch.save(model.state_dict(), save_filename)
            early_stopping_step = 0

        min_test_loss = min(min_test_loss, train_loss)
...
def evaluate_candidates(candidate_sets, model):
    ...
    with torch.no_grad():
        for sample_idx, candidate_set in tqdm(enumerate(candidate_sets), total=n_samples,
            ...
            for candidate_idx, x in enumerate(candidate_set):
